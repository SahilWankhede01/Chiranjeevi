...
const Navbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 glass-panel border-b border-slate-100/80 dark:border-zinc-800/60 bg-white/80 dark:bg-zinc-950/80 text-slate-800 dark:text-zinc-100 shadow-lg shadow-slate-100/20 dark:shadow-none transition-all duration-500">
        ...
        <div className="flex justify-between h-20">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col">
                <span className="font-serif font-extrabold text-xl tracking-wide text-ayurveda-green-800 dark:text-zinc-550 leading-tight group-hover:text-ayurveda-green-700 dark:group-hover:text-ayurveda-green-400 transition-colors">
                  {t('clinicName')}
                </span>
                <span className="text-[9px] font-extrabold text-ayurveda-saffron-600 dark:text-ayurveda-saffron-400 uppercase tracking-[0.2em] mt-0.5">
                  {t('clinicSub')}
                </span>
              </div>
            </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => {
              ...
              if (isTreatments) {
                return (
                  <div key={link.name} className="relative group min-h-[48px] flex items-center">
                    <a
                      href="#treatments"
                      className={`relative font-bold text-sm hover:text-ayurveda-green-600 dark:hover:text-ayurveda-green-400 py-3.5 flex items-center min-h-[48px] transition-colors ${isActive ? 'text-ayurveda-green-600 dark:text-ayurveda-green-400' : 'text-slate-600 dark:text-zinc-350'}`}
                    >
                      <span>{link.name}</span>
                      <ChevronDown size={14} className="ml-1 transition-transform duration-300 group-hover:rotate-180" />
                      <span className={`absolute bottom-2 left-0 h-[2px] bg-ayurveda-green-600 dark:bg-ayurveda-green-450 transition-all duration-300 ${isActive ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                    </a>
                    {/* Treatments Dropdown Menu */}
                    <div className="absolute top-[100%] left-1/2 -translate-x-1/2 mt-1 w-56 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/85 shadow-2xl py-2.5 z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                      {[
              ...
              return (
                <a
                  ...
                  className={`relative font-bold text-sm hover:text-ayurveda-green-600 dark:hover:text-ayurveda-green-400 py-3.5 flex items-center min-h-[48px] group transition-colors ${isActive ? 'text-ayurveda-green-600 dark:text-ayurveda-green-400' : 'text-slate-600 dark:text-zinc-350'}`}
                >
                  <span>{link.name}</span>
                  <span className={`absolute bottom-2 left-0 h-[2px] bg-ayurveda-green-600 dark:bg-ayurveda-green-450 transition-all duration-300 ${isActive ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </a>
            ...
            <Link
              ...
              className="bg-gradient-to-r from-ayurveda-green-600 to-ayurveda-green-700 hover:from-ayurveda-green-700 hover:to-ayurveda-green-800 text-white font-bold text-[11px] px-6 py-3.5 rounded-full shadow-lg shadow-emerald-700/10 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200 uppercase tracking-widest min-h-[48px] flex items-center justify-center"
            >
              {t('bookNow')}
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            <button
              ...
              className="min-h-[48px] min-w-[48px] flex items-center justify-center rounded-xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-500 dark:text-zinc-400 transition-all duration-300 hover:scale-105 active:scale-95"
              title="Toggle Theme"
            >
              {theme === 'dark' ? (
                <Sun size={18} className="text-amber-405 rotate-0 transition-transform duration-500 hover:rotate-45" />
              ) : (
            ...
            <div className="relative" ref={langRef}>
              <button
                ...
                className="px-4 flex items-center gap-2 rounded-xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-600 dark:text-zinc-350 transition-all hover:scale-105 min-h-[48px]"
                title="Select Language"
              >
                <Globe size={16} className="text-slate-450 dark:text-zinc-500" />
            ...
            {user && user.role === 'doctor' && (
              <div className="relative" ref={notifRef}>
                <button
                  ...
                  className="min-h-[48px] min-w-[48px] flex items-center justify-center rounded-xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-555 dark:text-zinc-400 relative transition-all duration-300 hover:scale-105"
                  title="Notifications"
                >
                  <Bell size={18} />
                  {unreadCount > 0 && (
                      ...
                      <div className="flex items-center gap-2">
                        {unreadCount > 0 && (
                          <button
                            ...
                            className="text-[10px] text-ayurveda-green-600 dark:text-ayurveda-green-450 hover:underline font-extrabold"
                          >
                            Mark All Read
                          </button>
                        )}
                            ...
                            >
                              ...
                              {parsed ? (
                                <div className="text-[11px] space-y-1 mt-1 font-semibold text-slate-600 dark:text-zinc-400 pl-3.5">
                                  <p><span className="text-slate-400 dark:text-zinc-550">Patient:</span> {parsed.patientName}</p>
                                  <p><span className="text-slate-400 dark:text-zinc-550">Date & Slot:</span> {parsed.date} at {parsed.time}</p>
                                  <p className="truncate"><span className="text-slate-400 dark:text-zinc-550">Reason:</span> {parsed.reason}</p>
                                </div>
                              ) : (
            ...
            {user ? (
              <div className="relative" ref={profileRef}>
                <button
                  ...
                  className="flex items-center gap-2.5 px-4 rounded-full border border-slate-100 dark:border-zinc-800/80 hover:bg-slate-50 dark:hover:bg-zinc-900 transition-all focus:outline-none min-h-[48px]"
                >
                  {user.avatar ? (
                    <img
                      src={user.avatar}
                      ...
                      className="w-8 h-8 rounded-full object-cover border-2 border-white dark:border-zinc-950"
                    />
                  ) : (
                    <div className="w-8 h-8 rounded-full bg-ayurveda-green-100 dark:bg-zinc-850 text-ayurveda-green-700 dark:text-zinc-300 flex items-center justify-center font-bold text-xs">
                      {user.name[0]}
                    </div>
                  )}
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2.5 w-52 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 shadow-2xl py-2 z-50 animate-fade-in">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-2 px-4.5 py-2.5 text-xs text-slate-700 dark:text-zinc-350 font-bold hover:bg-slate-50 dark:hover:bg-zinc-800/50"
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-4.5 py-2.5 text-xs text-red-600 dark:text-red-400 font-bold hover:bg-slate-50 dark:hover:bg-zinc-800/50 border-t border-slate-100 dark:border-zinc-800/80 mt-1"
                    >
                      <LogOut size={14} />
                      {t('logout')}
                    </button>
              ...
              <div className="flex items-center space-x-2.5">
                <Link
                  ...
                  className="text-xs font-bold text-slate-600 dark:text-zinc-300 hover:text-ayurveda-green-600 px-4 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-900 transition-all duration-200 min-h-[48px] flex items-center justify-center"
                >
                  {t('login')}
                </Link>
                <Link
                  to="/register"
                  /* FIX 5: Tap target size - min-h-[48px] px-4 flex items-center ensures >48x48px hit bounds */
                  className="text-xs font-extrabold bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:text-zinc-200 px-4 rounded-xl border border-slate-200/50 dark:border-zinc-800/60 transition-all duration-200 min-h-[48px] flex items-center justify-center"
                >
                  {t('register')}
                </Link>
              </div>
          ...
          <div className="flex items-center md:hidden space-x-2.5">
            ...
            <button
              onClick={toggleTheme}
              /* FIX 5: Tap target size - min-h-[48px] min-w-[48px] flex items-center justify-center ensures >48x48px hit area */
              className="min-h-[48px] min-w-[48px] flex items-center justify-center rounded-xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 text-slate-555 dark:text-zinc-400"
            >
      ...
      {/* Mobile Drawer Menu */}
      {isOpen && (
        <div className="md:hidden border-t border-slate-150 dark:border-zinc-850 bg-white dark:bg-zinc-950 px-4 pt-2.5 pb-6 space-y-4 shadow-inner animate-fade-in">
          {navLinks.map((link) => {
            const isTreatments = link.path === '#treatments';
            const activeHash = location.hash || '#home';
            const isActive = link.path === activeHash || (link.path === '/' && activeHash === '#home') || (link.path === '/' && !location.hash);
          ...
          <div className="py-2 border-t border-slate-100 dark:border-zinc-850">
            ...
            <div className="flex gap-2 px-3">
              {[
                ...
                <button
                  key={lang.code}
                  onClick={() => selectLanguage(lang.code)}
                  className={`text-xs px-3.5 py-2 rounded-full font-bold border transition-all ${language === lang.code ? 'bg-ayurveda-green-600 text-white border-ayurveda-green-600' : 'bg-transparent border-slate-200 dark:border-zinc-850 text-slate-600 dark:text-zinc-350'}`}
                >
                  {lang.label}
                </button>
              ))}
          ...
          <div className="pt-3 border-t border-slate-100 dark:border-zinc-850 space-y-3">
            {user ? (
              <>
              ...
              <div className="grid grid-cols-2 gap-2 px-3">
                <Link
                  to="/login"
                  onClick={() => setIsOpen(false)}
                  className="text-center py-2.5 rounded-xl border border-slate-200 dark:border-zinc-850 text-sm font-bold text-slate-700 dark:text-zinc-300"
                >
                  {t('login')}
                </Link>
                <Link
